...
class KeyStat():
    ...
    def get_prob_same_keybits(self, n,  tau, alpha=None):
        ''''''
        if not alpha:
            alpha=self.alpha
    ...
    def plot_prob_matching_keys(self, n_list, tau_list):
        ...
        plt.ylabel('$P(\mathbf{{k}}_a = \mathbf{{k}}_b )$')
        plt.title(r"$p_{{ch}}$= {}".format(self.alpha),fontsize=11)
        plt.grid()
    ...
    def plot_exp_no_missmathces(self, n_list, tau_list):
        ...
        plt.xticks(n_list)
        plt.xlabel('Blocksize (n)')
        plt.ylabel("E(dist($\mathbf{{k}}'_a,\mathbf{{k}}'_b)$)")
        plt.title(r"$p_{{ch}}$= {}".format(self.alpha),fontsize=11)
        plt.grid()
        plt.legend()
        plt.tight_layout()
    ...
    def createListTau(self, n_list):
        ...
        n_stretcher = []
        for n in n_list:
            tau = [x for x in range(1, math.ceil(n/2))]
            n_stretcher.append(len(tau))
            if tau:
                list_tau_lists += [x for x in range(1, math.ceil(n/2))]
